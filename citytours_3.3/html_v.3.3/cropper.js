...
$('#cropBtn').on('click', function () {
    var imgData = document.createElement('img'); /*画像生成*/
    var cropImg = $('#img').cropper('getCroppedCanvas', {
        width: 560,
        height: 188,
    });
    
    imgData.setAttribute('src', cropImg.toDataURL()); //src属性がcropImg.toDataURL()に変更(ソースからデータURIに変更)

    $('#cropped').html(imgData); //htmlを返す。←これが欲しい！

    // umetani
    /*    var img = document.getElementById('cropped');
        img = img.children;
    var hidden = img.getAttribute("src");
    console.log(hidden);*/
    // img.setAttribute("src"));

    var hidden = $('#cropped > img').attr("src");
    console.log(hidden);
    $('#cropp_file_name').val(hidden);
